...
# Creates hardlink copies of UE5 Engine and symlinks for Content
# from an existing 1st environment.
#
# Usage:
#   carla-env.sh setup   <1st-carla-root>   # Engine + Content (recommended)
#   carla-env.sh engine  <1st-carla-root>   # Engine hardlink copy only
#   carla-env.sh content <1st-carla-root>   # Content symlink only
#   carla-env.sh check-update               # Check repos for updates
#
# Sharing model:
...
_write_config() {
    ...
    cat > "$conf_file" <<EOF
# Auto-generated by carla-env.sh on $(date -Iseconds)
...
_read_config() {
    ...
    if [ ! -f "$conf_file" ]; then
        ...
        _log_error "  Run 'carla-env.sh setup' first."
        return 1
    fi
...
_check_engine_path_env() {
    ...
    if [ -n "${CARLA_UNREAL_ENGINE_PATH:-}" ]; then
        ...
        _log_info "    source tools/activate_ue5_clone.sh"
    ...
    if [ ${#found_in[@]} -gt 0 ]; then
        ...
        _log_warn "    source tools/activate_ue5_clone.sh"
    fi
...
_show_help() {
    cat <<'EOF'
Usage: carla-env.sh <command> [<1st-carla-root>]
...
Examples:
  carla-env.sh setup /path/to/1stCarlaUE5
  carla-env.sh check-update
EOF
}
...
cmd_setup() {
    ...
    echo ""
    echo "Next steps:"
    echo "  1. source tools/activate_ue5_clone.sh"
    ...
    if [ -z "$engine_origin" ]; then
        ...
        (
            ...
            if [ -n "$new_commits" ]; then
                ...
                echo "    carla-env.sh engine $(dirname "${CONF_SOURCE_PROJECT}")" >&2
