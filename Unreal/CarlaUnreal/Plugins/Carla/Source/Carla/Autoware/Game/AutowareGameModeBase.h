...
public:
  ...
  UFUNCTION(Exec, Category = "Autoware Game Mode")
  void DebugShowSignals(bool enable);

  UFUNCTION(BlueprintCallable, Category = "Autoware Game Mode")
  ATrafficLightManager* GetTrafficLightManager();

  UFUNCTION(Category = "Autoware Game Mode", BlueprintCallable)
  const TArray<FTransform>& GetSpawnPointsTransforms() const{
    return SpawnPointsTransforms;
  }

  UFUNCTION(Category = "Autoware Game Mode", BlueprintCallable, CallInEditor, Exec)
  ...
  UFUNCTION(Category = "Autoware Game Mode")
...
private:
  ...
  UPROPERTY(Category = "Autoware Game Mode", EditAnywhere)
