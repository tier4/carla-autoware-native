...
#
# Usage:
#   Util/BuildDependencyShare/build-dependency-share.sh setup   <1st-carla-root>   # Engine + Content + RGL
#   Util/BuildDependencyShare/build-dependency-share.sh engine  <1st-carla-root>   # Engine hardlink copy only
#   Util/BuildDependencyShare/build-dependency-share.sh content <1st-carla-root>   # Content symlink only
#   Util/BuildDependencyShare/build-dependency-share.sh check-update               # Check repos for updates
#
# Sharing model:
...
# Write build-dependency-share.conf to the BuildShare directory.
...
_write_config() {
    ...
    local conf_file="$SCRIPT_DIR/build-dependency-share.conf"
    ...
    cat > "$conf_file" <<EOF
# Auto-generated by build-dependency-share.sh on $(date -Iseconds)
...
# Read build-dependency-share.conf. Sets global variables:
#   CONF_SOURCE_PROJECT, CONF_ENGINE_SOURCE, CONF_ENGINE_CLONE, CONF_CONTENT_SOURCE
# Read build-dependency-share.conf if it exists. Returns 1 if not found (non-fatal).
_read_config() {
    local conf_file="$SCRIPT_DIR/build-dependency-share.conf"
    CONF_SOURCE_PROJECT=""
    CONF_ENGINE_SOURCE=""
...
_check_engine_path_env() {
    ...
    if [ -n "${CARLA_UNREAL_ENGINE_PATH:-}" ]; then
        ...
        _log_info "    source Util/BuildDependencyShare/activate.sh"
    ...
    if [ ${#found_in[@]} -gt 0 ]; then
        ...
        _log_warn "    source Util/BuildDependencyShare/activate.sh"
    fi
...
_show_help() {
    cat <<'EOF'
Usage: build-dependency-share.sh <command> [<1st-carla-root>]
...
Examples:
  build-dependency-share.sh setup /path/to/1stCarlaUE5
  build-dependency-share.sh check-update
EOF
}
...
cmd_setup() {
    ...
    echo ""
    echo "Next steps:"
    echo "  1. source Util/BuildDependencyShare/activate.sh"
...
cmd_check_update() {
    ...
    # Fallback: try sibling directory (original environment without build-dependency-share.conf)
        ...
        (
            ...
            if [ -n "$new_commits" ]; then
                ...
                if [ -n "${CONF_ENGINE_CLONE:-}" ]; then
                    ...
                    echo "    build-dependency-share.sh engine $(dirname "${CONF_SOURCE_PROJECT}")" >&2
