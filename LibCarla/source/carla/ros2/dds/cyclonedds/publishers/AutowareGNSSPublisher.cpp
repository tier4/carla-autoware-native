...
bool AutowareGNSSPublisher::Init(DomainId domain_id) {
  ...
  if (const auto custom = ValidTopicName("/pose")) {
    pose_topic_name = custom.value();
  }
  ...
  if (const auto custom = ValidTopicName("/pose_with_covariance")) {
    pwcs_topic_name = custom.value();
  }
