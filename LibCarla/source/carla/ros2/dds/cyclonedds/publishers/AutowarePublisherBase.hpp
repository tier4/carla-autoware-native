...
/**
 * @brief Template base class for Autoware report publishers (CycloneDDS version).
 *
 * @tparam Message   The CycloneDDS C struct type (e.g. autoware_vehicle_msgs_msg_VelocityReport)
 * @tparam TypeName  A compile-time string constant holding the ROS2 type name
 *                   (e.g. "autoware_vehicle_msgs::msg::VelocityReport") used for the
 *                   CycloneDDS type registry lookup.  CycloneDDS has no PubSubType;
 *                   the type name is passed directly to CreateDDSPublisher().
 */
namespace carla {
namespace ros2 {

template<typename Message, const char* TypeName>
...
public:
  ...
  {
    // CycloneDDS: pass the type name string directly (no PubSubType object).
    _impl->_dds = CreateDDSPublisher(TypeName);
